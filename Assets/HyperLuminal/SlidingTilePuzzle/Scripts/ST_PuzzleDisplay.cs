...
public class ST_PuzzleDisplay : MonoBehaviour
{
    ...
    public GameObject GameControllerAudioSource;
    public GameObject puzzleactiveScript;
    public GameObject puzzleCompleteCanvas;
    ...
    private void Start()
    {
        audio = GameControllerAudioSource.GetComponent<AudioSource>();
    ...
    public IEnumerator CheckForComplete()
    {
        ...
        // if we are still complete then all th1e tiles are correct.
        if (Complete)
        {
            audio.Stop();
            ...
            puzzleactiveScript.SetActive(false);
            yield return new WaitForSeconds(1);
            puzzleCompleteCanvas.SetActive(true);

            PuzzleTimeTextGameObject.SetActive(false);
            Debug.Log("Puzzle Complete!");
            gameObject.SetActive(false);
            Destroy(kapi);
            // ballObject.isKinematic = false;
            (cameraObject.GetComponent(typeof(FreeCamera)) as FreeCamera).enabled = true;
